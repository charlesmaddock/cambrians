...
class Animal:
	...
	def __init__(self, x, y, color, name, sizeRadius, hidden_sizes, weights, remove_synapse_rate, eye_rot, FOV, view_distance, build_chance, protein_metabolism, gen, mutation_rate, parents, mutations):
		...
		self.birth_timer_max = 5
		self.sizeRadius = mutate_organ(sizeRadius, 1, 9, 20)
	...
	def check_animal_mouth_collision( self, animal, x0, y0, x1, y1, animal_rad):
		...
		if animal_rad + self.mouth_radius >= distance:
			...
			if self.acc_factor > 0.3 and self.mouth_len > self.sizeRadius:
				...
				if self.protein_metabolism == 1:
					animal.health -= (abs((self.acc_factor - self.deacc_factor)) * self.sizeRadius * 2) + 0.4
	...
	def think(self):
		...
		self.acc_factor = pow(outputs[0]*1.2 ,2)
	...
	def update(self):
		...
		self.acc.x = math.cos(math.radians(self.rot)) * ((self.acc_factor - self.deacc_factor)) / 3
		self.acc.y = math.sin(math.radians(self.rot)) * ((self.acc_factor - self.deacc_factor)) / 3
		...
		if self.energy > (pow(self.sizeRadius, 2) * 3.14 ) and self.birth_timer > (frames_per_second * self.birth_timer_max) and self.birth_p > 0.3:
			self.birthed_kin += 1
